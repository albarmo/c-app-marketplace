...
export function DrawerHeader({ children, onCloseAction }: { children: React.ReactNode, onCloseAction: () => void }) {
    return (
        <section className='flex items-center justify-between p-3.5 border-b'>
            {children}
            <button onClick={() => onCloseAction()} className='size-10 flex justify-center items-center'>
...
export function DrawerContent({ children }: PropsWithChildren) {
    return (
        <section className='p-3.5 py-1.5'>
            {children}
        </section>
    )
}
...
export function DrawerFooter({ children }: PropsWithChildren) {
    return (
        <section className='p-3.5 border-t'>
            {children}
        </section>
    )
}
...
export default function ModalBottomDrawer({ children, isOpen }: I_PropsBottomDrawer) {
    if (!isOpen) return <></>
